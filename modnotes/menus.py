...
class Page:
    ...
    async def format_page(self, menu: Menu, page: list) -> Union[discord.Embed, str]:
        ...
        if await ctx.embed_requested():
            ...
            for data in page:
                try:
                    ret.add_field(name=data[0], value=f"{data[1]}\n<t:{int(data[2])}:f> (<t:{int(data[2])}:R>)", inline=False)
                except:
                    ret.add_field(name=data[0], value=data[1], inline=False)
        return ret
...
class Menu(discord.ui.View):
    ...
    async def _get_kwargs_from_page(self, page: list) -> dict:
