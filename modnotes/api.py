...
class NoteApi:
    ...
    async def create_note(
        ...
        async with self.config.member(user).notes() as notes:
            ...
            notes.append({"author": moderator.id, "note": note, "case_number": case_num, "time": time.time()})
    ...
    async def clear_notes(
        ...
        async with self.config.member(user).notes() as notes:
            ...
            notes.clear()
        log.debug(
            f"Moderator {moderator.name} ({moderator.id}) cleared {user.name}'s ({user.id}) notes"
        )
